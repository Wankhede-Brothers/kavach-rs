...
fn intentional_marker_with_upgrade_trigger_is_clean() {
    let (note, has_trigger) =
        marker_note("// kavach:intentional cohesive hub; upgrade when it exceeds 3 concerns")
            .unwrap();
...
fn when_clause_counts_as_trigger() {
    let (_n, has_trigger) =
        marker_note("// kavach:intentional single impl; split when a 2nd caller appears").unwrap();
    assert!(has_trigger);
}
