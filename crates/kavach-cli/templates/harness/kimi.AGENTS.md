...
<!-- Install at <HOME>/.kimi-code/AGENTS.md so every project inherits it. Ultra-compact action imperatives; kavach gates + DB extend, never relax. -->
...
- **PATH_POLICY empty file_path** → hook payload lost the path; do NOT pivot to Bash → ensure the Write/Edit call carries a real `file_path` → if the payload is correct and the gate still denies, fix the Kavach vendor lowering at root (`kavach heal capture`) → retry Write/Edit.
- **WRITE_BYPASS** → Bash was about to write a tracked source file, bypassing pre-write guards → use Write/Edit tool, not `>`/`sed -i`/`tee`/`python open()` → retry.
...
4. **Execute directly.** Kimi runs single-agent to stay inside token limits. Do Read/Edit/Write/Bash work yourself rather than spawning subagents. Frontier tokens = decision + verification + synthesis; use cheap local tools (rg/fd/bat/sd/jaq) for labor. Independent non-interfering tool calls may still run in parallel in one response.
