...
| `bloatware_guard` (§bloatware no-tombstone) | PreWrite | **P0→block** — a governed source file (`.rs`/`.sql` in `crates/{core,api,services,kavach-*}`) adds a COMMENT documenting a removal (`removed`/`abolished`/`deprecated`/`no longer`/`dropped below`/`(formerly`/`used to be`/`historical on-disk`); routed via `guards2026::bloatware` (after `dedup`, before `webhook`). Exactness keeps FP-set empty: only a comment-prefixed line fires — a removal word in a string literal or identifier never does. FP-bound proven in `bloatware_guard_test.rs`; engine-entry test `guards2026_test.rs`. SOURCE: decision.bloatware.no-tombstone-comments. |
...
| `bulk_op_guard` (`blocklist.rs::bulk_op_advisory`) | PreToolUse:Bash | **P1Advisory** — a bulk multi-file mutation typed inline (`rnr` batch-rename · `sg`/`ast-grep` structural rewrite · `fd -x sd` / `rg -l \| xargs sd` fan-out · `sd -i` across ≥2 explicit paths) emits `[BULK_VIA_SCRIPT]` steering it to ONE committed `scripts/<verb>.sh` run via a `just <verb>` recipe. Complement (not contradiction) to `write_bypass`: that DENIES laundering ONE source edit; this NUDGES the bulk SHAPE the launder-guard lets through (rnr/sg rename, fd-x). NEVER blocks. Carve: a `just` recipe or `bash scripts/*.sh` is the sanctioned path and never fires. Floor + `gate.bulk_op_vocab` graph overlay (mutator/marker set is DB-tunable, fail-closed to the compiled floor). FP-bound proven in `bulk_op_guard_test.rs` (single-file edit + read-fan-out + just-carve silent); engine-entry test `blocklist_test.rs::bulk_rename_via_rnr_*`. SOURCE: decision.bulk.one-script-not-n-edits. |
| `tdd_guard` (`pre_write_guards/tdd_guard.rs`) | PreWrite (runs FIRST in chain) | **P0→block** — Red-Green TDD: a production `.rs` Write/Edit is DENIED unless the unit's SEPARATE test file (`{stem}_test.rs`/`{stem}/tests.rs` via `#[path]`/`tests/{stem}.rs`) was touched earlier THIS turn. Also DENIES an inline `#[test]`/`#[cfg(test)] mod` in a production file (tests must live in a separate mapped file). Carve-outs: test paths, non-code. Escape `KAVACH_TDD_BYPASS=1`. Built test-first; FP-bound in `tdd_guard/tests.rs`. SOURCE: decision.tdd.test-first-separate-file-block |
...
| `lint_profile_guard` (`kavach-patterns/src/lint_profile_guard.rs`) | PreWrite (`advisories::collect`) | **P1Advisory** — a source edit (`.rs/.ts/.tsx/.js/.jsx/.go`) lands in a project that HAS a stack manifest (Cargo.toml / package.json / go.mod) but NO strict-rules profile (no `[workspace.lints`/`[lints` for Rust, no tsconfig.json for TS, no `.golangci.yml` for Go) → emits `[LINT_PROFILE_P1]` nudging `kavach lint init`. Walks UP from the file to the nearest manifest; fail-soft (no manifest / read error → silent, never blocks). Carve-outs: test/non-source. FP-bound in `lint_profile_guard_test.rs` (7 cases). SOURCE: decision.lint.language-profile-template. |
| `classify_kavach_misuse` (`post_tool_bash/detect.rs`, wired in `handle.rs`) | PostToolUse:Bash | **P1Advisory** — a failed `kavach …` call whose clap stderr says "unrecognized subcommand"/"unexpected argument" injects `[KAVACH_MISUSE]` (run `kavach commands --tree` / `kavach <sub> --help`, never fabricate). If the rejected token IS a real shipped verb (`KAVACH_VERBS` allow-list, kept in sync by hand with `cli.rs::Commands`; a stale list degrades safely — a missed verb falls through to the `[KAVACH_MISUSE]` consult-help advisory, still correct), the binary lags source → `[KAVACH_STALE_BINARY]` (run `just install` to rebuild+reinstall, then retry). Fires ONLY when cmd starts with `kavach` AND output carries a clap usage marker — a success / runtime-DB error / non-kavach command stays silent. FP-bound in `tests/classify.rs` (verb/flag/stale + 4 negatives). SOURCE: decision.kavach-misuse-recovery. |
