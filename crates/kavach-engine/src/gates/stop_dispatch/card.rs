...
/// a transient daemon blip would strand the operator's own card. The holder-vs-foreign
...
pub(crate) fn claim_card(project_slug: &str, key: &str) -> bool {
    ...
    // Carry this session's id so the RPC fuses an occupancy lease (holder+TTL+
    // fence) onto the won claim — without an operator a hung session's card cannot
