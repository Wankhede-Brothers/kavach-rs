...
/// Run the objective build+test witnesses ONCE over the whole workspace. CWD is
/// the agent's project root; a Rust workspace at the root OR an immediate subdir
/// (monorepo) is verified, else `KAVACH_VERIFY_CMD`, else `Unprovable`.
pub(crate) fn run_workspace_witnesses() -> WitnessRun {
    // E6 (cross-repo witness root): a card whose code lives in ANOTHER repo than
    // the dispatch CWD (e.g. a kavach-rs harness-self-improvement card dispatched
    // while CWD is the project's Backend) is verified in the WRONG workspace and
    // can never pass. `WITNESS_ROOT` lets such a card name its real repo; the gate
    // runs cargo THERE. Unset / not a workspace → fall through to CWD discovery.
