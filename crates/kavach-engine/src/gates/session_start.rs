...
pub(crate) fn run(input: &HookInput) -> Result<(), EngineError> {
    ...
    // Heavy boot work (skill-dir scan + fastembed RAG-tree rebuild) is skipped
    // under `cargo test`/`nextest` and when KAVACH_SKIP_HEAVY_BOOT=1: rebuilding
    // embeddings on every invocation would blow the per-test timeout under the
    // parallel workspace run (and is wasteful in CI). The gate's own logic still
    // runs; only the expensive registry/embedding refresh is bypassed.
