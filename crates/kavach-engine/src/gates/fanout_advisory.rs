...
pub(crate) fn nudge(session: &mut SessionState, tool_name: &str) -> Option<String> {
    if session.fanout_nudge_sent
        ...
        || !kavach_config::model::is_frontier_tier(&session.model_id)
    {
        return None;
    }
    ...
    Some(format!(
        "[FANOUT_NUDGE] You ran {tool_name} yourself on the frontier tier. You are the \
         ORCHESTRATOR — spawn a {} agent to do the read/edit/run, then VERIFY what it \
        ...
        kavach_config::model::CHEAP_EXECUTOR_TIER
    ))
}
