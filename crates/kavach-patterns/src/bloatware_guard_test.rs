...
fn tombstone_comment_is_p0_blocked() {
    ...
    let v = detect("crates/core/x.rs", "// owner-gating abolished 2026-06-20\nfn f() {}");
...
fn sql_dash_dash_tombstone_is_blocked() {
    ...
    let v = detect("crates/core/m.sql", "-- legacy field REMOVED; dropped below for old stores");
...
fn removal_word_in_string_literal_does_not_fire() {
    ...
    assert!(detect("crates/core/x.rs", src).is_empty(), "string literal is not a tombstone");
}
...
fn removal_word_in_identifier_does_not_fire() {
    ...
    assert!(detect("crates/core/x.rs", src).is_empty(), "identifier is not a tombstone");
}
...
fn ordinary_explanatory_comment_does_not_fire() {
    ...
    assert!(detect("crates/core/x.rs", src).is_empty(), "explanatory comment is fine");
}
...
fn non_governed_path_is_exempt() {
    ...
    assert!(detect("docs/notes.md", src).is_empty(), "non-source/non-governed exempt");
}
