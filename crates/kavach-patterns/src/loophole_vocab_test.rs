...
fn floor_is_never_empty() {
    ...
    assert!(
        !v.trigger_markers().is_empty(),
        "trigger floor must not be empty"
    );
    assert!(
        !v.dimensions.is_empty(),
        "dimension floor must not be empty"
    );
}
...
fn graph_overlay_adds_dimension_floor_intact() {
    ...
    assert_eq!(
        dimension_for_marker(&v, "ignore previous").as_deref(),
        Some("prompt-injection")
    );
    assert_eq!(
        dimension_for_marker(&v, "os.system").as_deref(),
        Some("injection"),
        "floor intact"
    );
}
