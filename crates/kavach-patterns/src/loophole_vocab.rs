...
fn access_money_dims() -> Vec<DimensionRule> {
    vec![
        dim(
            "authz",
            "authentication authorization session token idor loophole lens",
            &[
                "auth",
                "token",
                "session",
        ...
        dim(
            "idor",
            "authorization bypass user-controlled-key idor ownership loophole lens",
            &[
                "find_by_id",
                "findById",
                "get_object_or_404",
        ...
        dim(
            "crypto",
            "crypto key-management nonce-reuse weak-algorithm loophole lens",
            &[
...
fn injection_web_dims() -> Vec<DimensionRule> {
    vec![
        ...
        dim(
            "ssrf",
            "ssrf outbound-request url-validation dns-rebinding loophole lens",
            &[
                "reqwest::get",
                "fetch(",
                "axios",
        ...
        dim(
            "xss",
            "xss output-encoding html-escaping dom-sink loophole lens",
            &[
                "innerHTML",
        ...
        dim(
            "csrf",
            "csrf state-change samesite anti-forgery-token loophole lens",
            &["csrf", "samesite", "@csrf_exempt", "X-CSRF"],
        ),
        dim(
            "deserialization",
            "deserialization untrusted-input gadget loophole lens",
            &[
                "deserialize",
                "from_str",
                "pickle.load",
        ...
        dim(
            "path-traversal",
            "path-traversal symlink canonicalization loophole lens",
            &[
                "canonicalize",
                "../",
                "os.path.join",
...
fn runtime_supply_dims() -> Vec<DimensionRule> {
    vec![
        ...
        dim(
            "integer-overflow",
            "integer-overflow truncation wrapping-cast loophole lens",
            &[
                "wrapping_",
                " as u",
                " as i",
        ...
        dim(
            "resource-exhaustion",
            "resource-exhaustion dos unbounded backpressure loophole lens",
            &[
                "unbounded",
                "while True",
                "loop {",
        ...
        dim(
            "upload",
            "file-upload unrestricted-type size-limit content-type loophole lens",
            &[
                "multipart",
                "FileUpload",
                "request.files",
        ...
        dim(
            "supply-chain",
            "supply-chain dependency integrity unpinned-version loophole lens",
            &[
                "git dependency",
                "latest",
                "curl | sh",
        ...
        dim(
            "information-leak",
            "information-leak secret-in-log error-detail pii loophole lens",
            &[
                "println!",
                "console.log",
                "printStackTrace",
        ...
        dim(
            "logging",
            "security-logging audit-event alerting missing-log loophole lens",
            &[
                "audit_log",
                "security_event",
                "auth_failure",
