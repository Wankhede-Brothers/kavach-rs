...
use super::{AuditVerdict, detect_reward_hacking, first_floor_violation, safety_floor_held};
...
fn a_hard_block_may_never_be_relaxed_by_the_learned_policy() {
    ...
    assert!(
        !safety_floor_held(Action::Block, Action::Allow),
        "Block→Allow is a relaxation"
    );
    assert!(
        !safety_floor_held(Action::Block, Action::Ask),
        "Block→Ask is a relaxation"
    );
}
...
fn matching_or_tightening_a_block_holds_the_floor() {
    assert!(
        safety_floor_held(Action::Block, Action::Block),
        "Block→Block holds"
    );
}
...
fn advisory_rule_verdicts_are_free_to_tune_either_direction() {
    ...
    for rule in [Action::Allow, Action::Ask] {
        for shadow in [Action::Allow, Action::Ask, Action::Block] {
            assert!(
                safety_floor_held(rule, shadow),
                "{rule:?}→{shadow:?} is advisory tuning"
            );
        }
    }
}
...
fn batch_audit_returns_the_first_floor_violation() {
    ...
    assert_eq!(
        first_floor_violation(&pairs),
        Some((Action::Block, Action::Allow))
    );
}
...
fn soft_tracking_hard_within_tolerance_is_healthy() {
    ...
    assert_eq!(
        detect_reward_hacking(&hard, &soft, 0.05),
