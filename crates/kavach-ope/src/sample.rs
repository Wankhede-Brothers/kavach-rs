...
impl LoggedSample {
    /// Construct a sample. Propensity is clamped into `(0, 1]` — a non-positive
    /// logged propensity is a logging bug, but clamping keeps IPS finite rather
    /// than dividing by zero (fail-closed: such a sample contributes ~its reward
    /// at weight 1, never an infinite spike).
    #[must_use]
