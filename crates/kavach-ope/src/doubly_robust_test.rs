...
fn empty_is_non_gating() {
    ...
    let model = ConstByAction { allow: 0.0, ask: 0.0, block: 0.0 };
...
fn exact_model_makes_the_correction_vanish() {
    ...
    let model = ConstByAction { allow: 1.0, ask: -1.0, block: 0.0 };
...
fn correct_propensities_fix_a_biased_model() {
    ...
    let zero_model = ConstByAction { allow: 0.0, ask: 0.0, block: 0.0 };
    let est = estimate(&samples, &policy, &zero_model);
    // baseline 0 + 1*(r - 0) = r; mean(1,0,1,0) = 0.5.
    assert_eq!(est.value, 0.5, "correct propensities recover the value despite a zero model");
}
