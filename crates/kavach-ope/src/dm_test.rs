...
fn empty_input_is_non_gating() {
    ...
    let model = ConstByAction { allow: 1.0, ask: 0.0, block: 0.0 };
...
fn dm_value_is_the_model_prediction_not_the_logged_reward() {
    ...
    let policy = FixedPolicy { allow: 1.0, ask: 0.0, block: 0.0 };
    let model = ConstByAction { allow: 1.0, ask: -1.0, block: -1.0 };
...
fn dm_mixes_actions_by_the_target_policy_probabilities() {
    ...
    let samples = vec![sample(Action::Allow), sample(Action::Allow), sample(Action::Allow)];
    let policy = FixedPolicy { allow: 0.5, ask: 0.5, block: 0.0 };
    let model = ConstByAction { allow: 2.0, ask: 0.0, block: -10.0 };
    let est = estimate(&samples, &policy, &model);
