...
//
// SINGLE-WRITER INVARIANT: this path MUST go through the kavach-rpc daemon,
...
// contention SIGTERMs the running daemon and races for the lock — from a
...
// daemon, the one process that already holds the exclusive lock.
...
/// Record a mistake via the kavach-rpc daemon (the single `RocksDB` writer).
///
...
/// # Errors
///
/// Returns an error string if the daemon is unreachable or the RPC itself
