...
fn stop_continue() -> HookResponse {
    let mut resp =
        HookResponse::new_block("[AUTO_CONTINUE] Kanban has pending work — do not stop.");
...
fn auto_continue_reaches_cursor_stop_as_followup_message() {
    ...
    assert!(
        json.contains("AUTO_CONTINUE"),
        "cursor carries continuation: {json}"
    );
    assert!(
        !json.contains(r#""continue""#),
        "cursor stop has no `continue` field: {json}"
    );
}
...
fn auto_continue_reaches_codex_stop_body() {
    ...
    assert!(
        json.contains("AUTO_CONTINUE"),
        "codex carries continuation: {json}"
    );
}
...
fn clean_stop_does_not_resubmit_on_cursor() {
    ...
    assert!(
        ...
        "clean stop must not resubmit: {json}"
    );
}
