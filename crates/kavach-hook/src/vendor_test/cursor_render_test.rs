...
fn cursor_pre_tool_deny_renders_permission_deny() {
    ...
    assert!(
        !out.contains(r#""continue""#),
        "pre-tool has no continue field: {out}"
    );
...
fn cursor_submit_emits_continue_only_no_agent_message() {
    ...
    assert!(
        !json.contains("agent_message"),
        "submit honors no agent_message: {json}"
    );
    assert!(
        !json.contains("MISTAKE_LEDGER"),
...
fn cursor_after_file_edit_emits_empty_object() {
    ...
    assert_eq!(
        json, "{}",
        "afterFileEdit output is an empty object: {json}"
    );
}
...
fn cursor_pre_tool_allow_prefers_additional_context_over_allow_reason() {
    ...
    assert!(
        ...
        "shadow must reach agent_message: {json}"
    );
    assert!(
        json.contains("[LOOP]"),
        "LOOP compact must reach agent_message: {json}"
    );
    assert!(
