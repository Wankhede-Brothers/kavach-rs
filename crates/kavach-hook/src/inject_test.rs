// TDD red phase: pins the `compact_inject` API in crates/kavach-hook/src/inject.rs (not yet
...
use crate::inject::compact_inject;

#[test]
fn compact_inject_returns_compressed_text() {
    let input = "the gate is binding then retry";
    let output = compact_inject(input);
    assert!(
...
#[test]
fn compact_inject_preserves_protected_tokens() {
...
#[test]
fn compact_inject_never_panics_on_empty() {
    let output = compact_inject("");
    assert!(
        output.is_empty(),
...
#[test]
fn compact_inject_is_idempotent_on_preserved_only() {
    let input = "`code`";
    let output = compact_inject(input);
    assert_eq!(
        output, input,
