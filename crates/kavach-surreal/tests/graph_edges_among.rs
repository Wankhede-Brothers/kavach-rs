...
use kavach_surreal::{graph_list_edges_among, graph_relate_dynamic, graph_upsert_entity, open_memory};
...
async fn list_edges_among_keeps_intra_set_drops_boundary() {
    ...
    let a = graph_upsert_entity(&db, "concept", "alpha").await.expect("a");
    let b = graph_upsert_entity(&db, "concept", "beta").await.expect("b");
    let c = graph_upsert_entity(&db, "concept", "gamma").await.expect("c");
    let outside = graph_upsert_entity(&db, "concept", "outside")
        .await
        .expect("outside");

    // a->b and b->c are intra-set; c->outside crosses the visible boundary.
    graph_relate_dynamic(&db, &a, &b, "is_a", 1.0).await.expect("a->b");
    graph_relate_dynamic(&db, &b, &c, "part_of", 1.0).await.expect("b->c");
...
async fn list_edges_among_short_sets_yield_nothing() {
    ...
    let db = open_memory().await.expect("memory db");
    kavach_surreal::apply_schema(&db).await.expect("schema");
    let lone = graph_upsert_entity(&db, "concept", "solo").await.expect("solo");
    assert!(
        graph_list_edges_among(&db, &[lone]).await.expect("one").is_empty(),
        "single node -> no edges"
    );
    assert!(
        graph_list_edges_among(&db, &[]).await.expect("zero").is_empty(),
        "empty set -> no edges"
    );
}
