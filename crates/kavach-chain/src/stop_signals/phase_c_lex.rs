...
// NEG: suppress quoted/illustrative or meta-about-the-gate use (the string-literal-
// prose FP class). See decision.engine.anti-argue-meta-neg-arm.
const USER_REPORT_DISMISSAL_NEG: &str = r#"(?i)\bdetector\b|\b(?:root\s+)?cause\s+is\b|\bblocker\s+is\b|["'`“”][^"'`“”]*\b(?:correct|expected|fine|working|normal)\s+(?:behavior|as\s+designed)\b|\b(?:test|fixture|transcript|regex|gate|verbatim|quote[ds]?|example|illustrat)\w*\b"#;
